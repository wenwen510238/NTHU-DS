...
class CMAES_optimizer(Function):
    def __init__(self, target_func, seed=0, population_size=10, sigma=0.5):
        super().__init__(target_func)
        self.lower = self.f.lower(target_func)
        self.upper = self.f.upper(target_func)
        self.dim = self.f.dimension(target_func)
...
# import optuna
# class Optuna_optimizer(Function):
#     def __init__(self, target_func):
#         super().__init__(target_func)
#         self.lower = self.f.lower(target_func)
#         self.upper = self.f.upper(target_func)
...
#         study.optimize(objective, n_trials = FES)
...
if __name__ == '__main__':
    ...
    while func_num < 5:
        ...
        if func_num == 1: #dim = 6
            ...
            op = CMAES_optimizer(func_num, seed=256503, population_size=8)
            #pop size = 10, seed=62942 => 2.2330996191124502e-15
            #pop size = 8, seed=256503 => 7.688173188423398e-16
        elif func_num == 2: #dim = 2
            print("---------------------------------------------------------------------------")
            op = CMAES_optimizer(func_num, seed=122070, population_size=10)
            #pop size = 10, seed=122070 => 3.9711982803125572e-22
            #pop size = 50, seed=1096695 => 1.530391103778082e-21
        elif func_num == 3: #dim = 5
            print("---------------------------------------------------------------------------")
            # op = CoDE_optimizer(func_num)
            op = CMAES_optimizer(func_num, seed=295967, population_size=8)
            #pop size = 10, seed=124875 => 2.4553692412609962e-12
            #pop size = 8, seed=295967 => 1.666666804567285e-12
        elif func_num == 4: #dim = 10
            print("---------------------------------------------------------------------------")
            op = CMAES_optimizer(func_num, seed=227600, population_size=6)
            #pop size = 10, seed=128177 => 2.3092638912203256e-14
            #pop size = 6, seed=227600 => 8.881784197001252e-16

            # op = CoDE_optimizer(func_num)
            # op = EDA_LS_optimizer(func_num)

            # print("-------------------------------------------------------------")
            # print(op.evaluate(np.array([0.9999999965682178, 0.9999999364020274])))
            # print(op.evaluate(np.array([0.9999999990766839, 1.000000005143904])))
            # print("-------------------------------------------------------------")
        ...
        with open("410885034_function{}.txt".format(func_num), 'w+') as f:
