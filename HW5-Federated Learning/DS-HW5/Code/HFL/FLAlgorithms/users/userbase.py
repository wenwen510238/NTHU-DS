...
class User:
    ...
    def set_parameters(self, model, beta=1):
        ...
        for param_name, param_value in self.model.state_dict().items():
            if param_name in global_model_params:
                updated_param = beta * global_model_params[param_name] + (1 - beta) * param_value
                self.model.state_dict()[param_name].copy_(updated_param)
        # for param_name, param_value in self.model.state_dict().items():
        #     local_loss = self.local_loss
        #     beta_adjusted = min(beta * (1 + local_loss), 1) 

        #     if param_name in global_model_params:
        #         updated_param = (1-beta_adjusted) * global_model_params[param_name] + beta_adjusted * param_value
        #         self.model.state_dict()[param_name].copy_(updated_param)
